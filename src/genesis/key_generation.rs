...
impl KeyPair {
	...
	///private key to base58 wallet import format (WIF)
	pub fn private_key_base58(secret: SecretKey) -> String {
	...
	pub fn keypair_frombase58wif(secret: String) -> KeyPair {
		let context = &SECP256K1;
		let sk: Privkey = FromBase58::from_base58check(&secret).ok().expect("something wrong with wif");
		let the_secret = SecretKey::from_slice(context, &sk.key[..]).unwrap();
